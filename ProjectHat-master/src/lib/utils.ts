...
export function createMetadata(options: MetadataOptions): Metadata {
    const title = `${options.title} - MangaHat`;
    const description = `MangaHat - ${options.description}`;
    ...
    if (process.env.NEXT_PUBLIC_HOST && image && !image.startsWith("http")) {
        image = `https://${process.env.NEXT_PUBLIC_HOST}/${image.startsWith("/") ? image.slice(1) : image
            }`;
    }
    ...
    return {
        ...
        openGraph: {
            ...
            siteName: options.siteName ?? "MangaHat",
            images: image,
        },
        twitter: {
